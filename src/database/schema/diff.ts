...
//   const hasChangedStatus = (diff: { status: 'CHANGED' | 'UNCHANGED' }) => diff.status === statuses.CHANGED;
...
//     const hasChanged = [addedForeignKeys, updatedForeignKeys, removedForeignKeys].some((arr) => arr.length > 0);
