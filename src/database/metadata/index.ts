...
const hasComponentsOrDz = (model: Meta): model is ComponentLinkMeta => {
  return Object.values(model.attributes).some(({ type }) => types.isComponent(type) || types.isDynamicZone(type));
};
...
const createDynamicZone = (attributeName: string, attribute: Attribute, meta: ComponentLinkMeta) => {
