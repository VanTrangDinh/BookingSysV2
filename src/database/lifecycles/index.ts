...
export const createLifecyclesProvider = (db: Database): LifecycleProvider => {
  let subscribers = [subscriberUtils.timestampsLifecyclesSubscriber, subscriberUtils.modelsLifecyclesSubscriber];

  return {
    subscribe(subscriber) {
      assert(subscriberUtils.isValidSubscriber(subscriber), 'Invalid subscriber. Expected function or object');
