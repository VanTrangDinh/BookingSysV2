...
// import { replaceRegularRelations, cloneRegularRelations } from './relations/cloning/regular-relations';
...
// const isRecord = (value: unknown): value is Record<string, unknown> => isObject(value) && !isNil(value);
...
// const isValidObjectId = (value: unknown): value is Entity => isRecord(value) && 'id' in value && isValidId(value.id);
...
// const processData = (metadata: Meta, data: Record<string, unknown> = {}, { withDefaults = false } = {}) => {
...
//       const result = await this.createQueryBuilder(uid).init(params).first().execute<Entity | null>();
...
//       const res = await this.createQueryBuilder(uid).insert(dataToInsert).execute<Array<ID | { id: ID }>>();
...
//       const createdEntries = await this.createQueryBuilder(uid).insert(dataToInsert).execute<Array<ID | { id: ID }>>();
...
//       const updatedRows = await this.createQueryBuilder(uid).where(where).update(dataToUpdate).execute<number>();
...
//         mergeWith(data || {}, (original: unknown, override: unknown) => (override === null ? override : original)),
...
//           if (types.isRelationalAttribute(attr) && 'joinTable' in attr && attr.joinTable && !('component' in attr)) {
//             acc.push(attrName);
//           }
//           return acc;
//         }, [] as string[]);
...
//       const deletedRows = await this.createQueryBuilder(uid).where(where).delete().execute<number>();
